...
if [ -d /bedrock/strata/arch ] && [ -d /bedrock/strata/artix ]; then
	...
	sudo pacman -Syu --needed wine lutris prismlauncher flatpak plasma ufw qalculate-qt micro btop obs-studio filelight steam ark
	sudo strat -r arch pacman -Syu --needed obsidian code

else
	pacman -Syu --needed lutris prismlauncher wine-mono flatpak plasma ufw qalculate-qt code micro obsidian btop obs-studio filelight steam ark
